...
import {HttpContext, HttpContextToken, HttpErrorResponse, HttpEvent, HttpHandler, HttpInterceptor, HttpRequest, HttpStatusCode} from '@angular/common/http';
...
export class RestreamInterceptor implements HttpInterceptor {
  ...
  makeRequest(request: HttpRequest<unknown>, next: HttpHandler) {
    return this.addTokenToHeaders(request)
      .pipe(switchMap(req => {
  ...
  intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
    const handleError = (error: HttpErrorResponse) => {
      if (error.status === HttpStatusCode.Unauthorized) {
        return this.restreamAuth.refreshToken()
          .pipe(
